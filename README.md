...
# 安装 #
    wget https://raw.githubusercontent.com/Sincetoday-Lsp/Fail2ban/master/fail2ban.sh && chmod +x fail2ban.sh && bash fail2ban.sh 2>&1 | tee fail2ban.log
1. 第一步选择是否修改SSH端口。
1. 第二部输入最多尝试输入SSH连接密码的次数
1. 第三部输入每个恶意IP的封禁时间（单位：小时）

# 卸载 #
    wget https://raw.githubusercontent.com/Sincetoday-Lsp/Fail2ban/master/uninstall.sh && chmod +x uninstall.sh && bash uninstall.sh
